...
class SlipNetVpnService : VpnService() {
        ...
        connectJob = serviceScope.launch {
            ...
            try {
                ...
                when (currentTunnelType) {
                    ...
                    TunnelType.SHADOWSOCKS -> connectSingBox(profile, dnsServer)
                }
     ...
     */
         ...
         /**
     * Connect using sing-box based protocols (VLESS, Trojan, Hysteria2, Shadowsocks).
     * sing-box creates a local SOCKS5 proxy, then tun2socks routes traffic through it.
     *
     * For CDN-based configs (WebSocket/gRPC), auto-scans for clean IP if needed.
     *
     * Traffic flow:
     * App -> TUN -> hev-socks5-tunnel -> sing-box SOCKS5 (proxyPort)
     *   -> VLESS/Trojan/Hysteria2/SS -> Server -> Internet
     */
    private suspend fun connectSingBox(
        ...
        // ---- Proxy-only mode ----
        if (isProxyOnly) {
            val singboxResult = withContext(Dispatchers.IO) {
                SingBoxBridge.start(
            ...
            if (singboxResult.isFailure) {
                connectionManager.onVpnError(singboxResult.exceptionOrNull()?.message ?: "Failed to start sing-box")
                stopForeground(STOP_FOREGROUND_REMOVE)
                stopSelf()
                return
            }

            if (!waitForProxyReady(proxyPort, maxAttempts = 30, delayMs = 200)) {
                connectionManager.onVpnError("sing-box failed to start")
                SingBoxBridge.stop()
                stopForeground(STOP_FOREGROUND_REMOVE)
                stopSelf()
                return
    ...
    private suspend fun handleProxyStartupFailure(port: Int) {
        val nativeRunning = when (currentTunnelType) {
            ...
            TunnelType.TROJAN,
            TunnelType.HYSTERIA2,
            TunnelType.SHADOWSOCKS -> SingBoxBridge.isRunning()
        }
        ...
        serviceScope.launch {
            ...
            try {
                ...
                if (currentTunnelType == TunnelType.SSH) {
                    ...
                    if (!waitForNaiveReady(maxAttempts = 30, delayMs = 200)) {
                        Log.e(TAG, "NaiveProxy failed to restart on port $naivePort")
                        handleTunnelFailure("failed to reconnect NaiveProxy after network change")
                        return@launch
                    }

                    // Use server domain for SSH target (avoid Chromium loopback bypass)
                    val naiveSshHost = if (profile.sshHost == "127.0.0.1" || profile.sshHost.isBlank()) {
                        profile.domain
                    } else {
                        profile.sshHost
                ...
                // ========== sing-box protocols ==========
                } else if (currentTunnelType == TunnelType.VLESS ||
                           currentTunnelType == TunnelType.TROJAN ||
                           currentTunnelType == TunnelType.HYSTERIA2 ||
                           currentTunnelType == TunnelType.SHADOWSOCKS) {
                    ...
                    if (singboxResult.isFailure) {
                        Log.e(TAG, "Failed to restart sing-box after network change", singboxResult.exceptionOrNull())
                        handleTunnelFailure("failed to reconnect sing-box after network change")
                        return@launch
                    }

                    if (!waitForProxyReady(proxyPort, maxAttempts = 30, delayMs = 200)) {
                        Log.e(TAG, "sing-box failed to restart on port $proxyPort")
                        handleTunnelFailure("failed to reconnect sing-box after network change")
                        return@launch
                    }

                } else {
                    // DOH, SNOWFLAKE و سایر پروتکل‌ها
    ...
    private fun stopCurrentProxy() {
        ...
        when (currentTunnelType) {
            ...
            TunnelType.SHADOWSOCKS -> {
                Log.d(TAG, "Stopping sing-box (${currentTunnelType.displayName})")
                SingBoxBridge.stop()
            }
        }
    }
    ...
    private fun clearVpnServiceRef() {
        when (currentTunnelType) {
            ...
            TunnelType.TROJAN,
            TunnelType.HYSTERIA2,
            TunnelType.SHADOWSOCKS -> { /* sing-box: no bridge reference to clear */ }
        }
    }
    ...
    private fun isCurrentProxyHealthy(): Boolean {
        ...
        return when (currentTunnelType) {
            ...
            TunnelType.TROJAN,
            TunnelType.HYSTERIA2,
            TunnelType.SHADOWSOCKS -> SingBoxBridge.isClientHealthy()
        }
    }
