...
class ConfigImporter @Inject constructor() {
    ...
    companion object {
        ...
        private const val MODE_TROJAN = "trojan"
        private const val MODE_HYSTERIA2 = "hysteria2"
        private const val MODE_SHADOWSOCKS = "shadowsocks"
    ...
    fun parseAndImport(input: String): ImportResult {
        ...
        for ((index, line) in lines.withIndex()) {
            ...
            // Try direct proxy URIs first (vless://, trojan://, hy2://, ss://)
            val proxyProfile = SingBoxBridge.parseProxyUri(trimmedLine)
            if (proxyProfile != null) {
                profiles.add(proxyProfile)
                continue
            }

            // Then try slipnet:// format
    ...
    private fun parseProfileV2(fields: List<String>, lineNum: Int): ProfileParseResult {
        ...
        val tunnelType = when (tunnelTypeStr) {
            ...
            MODE_TROJAN -> TunnelType.TROJAN
            MODE_HYSTERIA2 -> TunnelType.HYSTERIA2
            MODE_SHADOWSOCKS -> TunnelType.SHADOWSOCKS
            else -> {
