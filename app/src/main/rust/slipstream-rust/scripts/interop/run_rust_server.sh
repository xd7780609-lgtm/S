...
RUN_DIR="${ROOT_DIR}/.interop/run-rust-server-$(date +%Y%m%d_%H%M%S)"
...
if [[ ! -x "${CLIENT_BIN}" ]]; then
  echo "Missing slipstream-client in ${BUILD_DIR}." >&2
  exit 1
fi
...
SLIPSTREAM_BUILD_DIR="${BUILD_DIR}" cargo build -p slipstream-server
...
echo "Waiting for C client to accept connections..."
sleep 2
success=0
for _ in $(seq 1 5); do
...
echo "Sending SIGTERM to Rust server..."
...
if [[ "${server_exited}" -ne 1 ]]; then
  echo "Rust server did not exit after SIGTERM; see logs in ${RUN_DIR}." >&2
  exit 1
fi
